...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        if (empty($request->slug)) {
            request()->session()->flash('error', 'Неверное изделие');
            return back();
        }
        ...
        if ($already_cart) {
            ...
            if ($already_cart->product->stock < $already_cart->quantity || $already_cart->product->stock <= 0) {
                return back()->with('error', 'Такого количества нет на складе!');
            }
            $already_cart->save();
        } else {
            $cart = new Cart;
            ...
            $cart->quantity = 1;
            $cart->amount = $product->discount ? ($product->price * (100 - $product->discount) / 100) * $cart->quantity : $product->price * $cart->quantity;
            if ($cart->product->stock < $cart->quantity || $cart->product->stock <= 0) {
                return back()->with('error', 'Такого количества нет на складе!');
            }
            $cart->save();
            Wishlist::where('user_id', auth()->user()->id)->where('product_id', $product->id)->update(['cart_id' => $cart->id]);
        }

        request()->session()->flash('success', 'Изделие успешно добавлено в корзину.');
        return back();
    }
    ...
    public function singleAddToCart(Request $request){
        ...
        if($already_cart) {
            ...
            if ($already_cart->product->stock < $already_cart->quantity || $already_cart->product->stock <= 0) return back()->with('error','Такого количества нет на складе!.');
    ...
    public function cartDelete(Request $request){
        ...
        request()->session()->flash('error','Ошибка, попробуйте ещё раз');
        return back();
    }
            ...
            foreach ($request->quant as $k=>$quant) {
                ...
                if($quant > 0 && $cart) {
                    ...
                    if($cart->product->stock < $quant){
                        request()->session()->flash('error','Нет в наличии');
                        return back();
                    }
                    ...
                    $error[] = 'Корзина недействительна!';
                }
            }
            return back()->with($error)->with('success', $success);
        }else{
            return back()->with('Корзина недействительна!');
        }
    }
