...
<div class="card">
    ...
    <div class="card-body">
      <form method="post" action="{{route('users.update',$user->id)}}">
        ...
        <input id="inputTitle" type="text" name="name" placeholder="Введите имя"  value="{{$user->name}}" class="form-control">
        @error('name')
        ...
        <div class="form-group">
          ...
          <input id="inputEmail" type="email" name="email" placeholder="Введите email"  value="{{$user->email}}" class="form-control">
          @error('email')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <select name="role" class="form-control">
                ...
                @foreach($roles as $role)
                    <option value="{{$role->role}}" {{(($role->role=='admin') ? 'selected' : '')}}>Админ</option>
                    <option value="{{$role->role}}" {{(($role->role=='user') ? 'selected' : '')}}>Пользователь</option>
                @endforeach
            </select>
          @error('role')
          <span class="text-danger">{{$message}}</span>
