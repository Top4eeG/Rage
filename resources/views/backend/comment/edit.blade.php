...
<div class="card">
  <h5 class="card-header">Редактирование комментария</h5>
  <div class="card-body">
    <form action="{{route('comment.update',$comment->id)}}" method="POST">
      ...
      <div class="form-group">
        ...
        <select name="status" id="" class="form-control">
          ...
          <option value="active" {{(($comment->status=='active')? 'selected' : '')}}>Активен</option>
          <option value="inactive" {{(($comment->status=='inactive')? 'selected' : '')}}>Неактивен</option>
        </select>
      </div>
