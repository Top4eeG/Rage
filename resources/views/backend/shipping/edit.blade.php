...
<div class="card">
    <h5 class="card-header">Редактирование доставки</h5>
    <div class="card-body">
      <form method="post" action="{{route('shipping.update',$shipping->id)}}">
        ...
        <input id="inputTitle" type="text" name="type" placeholder="Введите название"  value="{{$shipping->type}}" class="form-control">
        @error('title')
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            <option value="active" {{(($shipping->status=='active') ? 'selected' : '')}}>Активна</option>
            <option value="inactive" {{(($shipping->status=='inactive') ? 'selected' : '')}}>Неактивна</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
